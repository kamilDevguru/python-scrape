...
with open('result.csv', mode='w') as result:
    ...
    search_box = driver.find_element_by_id('CurrentSearchModel_FacilityType')
    search_box.click()

    # Select target item from select box
    first_key = driver.find_element_by_xpath(
        '//*[@id="CurrentSearchModel_FacilityType"]/option[3]')
    first_key.click()
    ...
    # Find table
    table = driver.find_element_by_xpath('//*[@id="MainContent"]/table/tbody')
    ...
    for record in records:
        ...
        try:
            new_row['administrator'] = first_info_list[3].split(':')[1].strip()
        except:
